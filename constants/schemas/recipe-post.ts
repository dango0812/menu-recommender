...
export const recipePostSchema = z.object({
  title: z.string().trim().min(2, '제목을 입력해 주세요').max(50, '제목은 50자 이하로 입력해 주세요.'),
  ...
  cookingTime: z
    .number()
    .int('조리 시간은 분 단위 정수로 입력해 주세요')
    .min(1, '1분 이상 입력해 주세요')
    .max(300, '300분 이하로 입력해 주세요'),
  mainIngredient: z.string().trim().min(1, '주재료를 입력해 주세요'),
  seasonings: z
    .array(
      z.object({
        name: z.string().trim().min(1, '양념을 입력해 주세요'),
        amount: z.string().trim().min(1, '양을 입력해 주세요'),
      })
    )
  ...
  cookingSteps: z
    .array(
      z.object({
        ...
        description: z.string().trim().min(1, '조리 설명을 입력해 주세요'),
